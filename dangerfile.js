if (danger.github.pr.body.length < 10) {
  warn('Please provide a detailed PR description. Explain the **what**, **why**, and **how**.')
}
...
if (invalidCommits.length > 0) {
  fail(`Some commits don't follow Conventional Commits:\n${invalidCommits.join('\n')}`)
}
...
if (hasSrcChanges && !hasTestChanges) {
  warn('This PR modifies source code but **no tests**. Consider adding tests.')
}
...
if (danger.github.pr.additions + danger.github.pr.deletions > BIG_PR_LINES_THRESHOLD) {
  fail(
...
const hasRequiredLabel = danger.github.pr.labels.some(label => REQUIRED_LABELS.includes(label.name))
